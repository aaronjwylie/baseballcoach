...
**Every message names its audience and where it lands** — `▣` on a screen, `✉`
by email. The audience settles *which* screen without a second icon: a customer
has no portal, so theirs is the public flow, and an admin's is the queue.
