...
**The trail writes three kinds of row, and the column only held one.** Sends
were listed; the **rung** each transition records and the **check** were not, so
"Payment cleared" appeared to write nothing when it writes `New`. A rung row is
attributed to the action that causes it rather than to the rung it names —
unlike an email, no substep exists whose whole job is "the status was written",
so it attaches to whoever moved it.
...
| **2 · Upload** | Attach a file | — | **Customer** ▤ "You can attach up to 5 files."<br>**Customer** ▤ "Files must be under 50 MB."<br>**Customer** ▤ "That file type isn't supported."<br>**Customer** ▤ "That file is empty."<br>**Customer** ▤ "Your session has expired. Please start again."<br>**Customer** ▤ "Please attach at least one file first." on trying to advance | `files attached — 3 intake` *(not built)* | **Customer** ▤ each file appears in the list with its size | At least one file attached |
...
|  | Upload the translated files | — | **Admin** ▣ “That file was rejected — too large, wrong type, or empty.” *(not built)* | `Translated` | **Admin** ▣ the files appear in the folder | Translated files uploaded |
...
| **8 · Reviewing** | Upload the response | — | **Coach** ▣ “That file was rejected — too large, wrong type, or empty.” *(not built)* | `Submitted` | **Coach** ▣ the file appears in their folder | Response uploaded |
...
|  | Upload the translation | — | **Admin** ▣ “That file was rejected — too large, wrong type, or empty.” *(not built)* | `Translated` | **Admin** ▣ the files appear in the folder | Translation uploaded |
