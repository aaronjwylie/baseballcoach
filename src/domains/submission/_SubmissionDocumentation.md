...
spent` is listed as itself, all five of it, rather than as "wrong code". Rows in
`code` are what the trail prints; the plain line above them is the condition in
the operator's words, which is a different sentence and stays that way.
