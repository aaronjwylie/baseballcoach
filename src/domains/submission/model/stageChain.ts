...
export const STAGE_CHAIN: Record<SubmissionStatus, ChainLine[]> = {
  draft: [
    { what: "Code sent to the customer", next: "Send the code", from: "①", passive: true, records: [...sendRecords("① code → customer")], failures: [...sendFailures("① code → customer")], toldOnFail: ["Customer/screen: \"That email address doesn't exist. Please check it for a typo and try again.\"", "Customer/screen: \"That inbox couldn't accept our email. It may be full, so please try a different address.\"", "Customer/screen: \"We couldn't deliver your code to that address. Check it for a typo, or try a different email.\"", "Customer/screen: \"We couldn't send your code — please check the address and try again.\"", "Customer/screen: \"We couldn't send your code — please try again in a moment.\""], toldOnSuccess: ["Customer/screen: \"Enter the code from your email.\"", "Customer/screen: \"We've sent a new code.\" on a resend"], met: sent("① code → customer") },
    { what: "Email proven", next: "Prove the email", from: "emailVerifiedAt", act: "waitCustomer", records: ["code accepted", ...Array.from({ length: 4 }, (_, i) => `code accepted — on attempt ${i + 2}`)], failures: [...WRONG_CODE, "code rejected — 5 attempts spent", "code rejected — the window had closed", "code rejected — no code outstanding"], toldOnFail: ["Customer/screen: \"That code doesn't match. Check the email and try again.\"", "Customer/screen: \"Too many incorrect attempts. Ask for a new code to try again.\"", "Customer/screen: \"We haven't sent a code yet. Ask for a new one below.\"", "Customer/screen: \"Too many attempts. Please wait a few minutes.\"", "Customer/screen: \"Too many code requests. Please wait a few minutes.\" on the resend"], toldOnSuccess: ["Customer/screen: the upload step opens — no message, the screen simply advances"], met: (s) => !!s.emailVerifiedAt },
  ],
  awaiting_payment: [
    { what: "At least one file attached", next: "Attach a file", from: "intake", toldOnFail: ["Customer/screen: \"You can attach up to 5 files.\"", "Customer/screen: \"Files must be under 50 MB.\"", "Customer/screen: \"That file type isn't supported.\"", "Customer/screen: \"That file is empty.\"", "Customer/screen: \"Your session has expired. Please start again.\"", "Customer/screen: \"Please attach at least one file first.\" on trying to advance"], toldOnSuccess: ["Customer/screen: each file appears in the list with its size"], met: has("intake") },
    { what: "Payment cleared", next: "Clear payment", from: "paidAt", act: "waitCustomer", failures: ["card declined → customer", ...sendFailures("card declined → customer"), "declined *(not built)* — only the notice is recorded, not the decline"], toldOnFail: ["Customer/screen: \"That card didn't go through\"", "Customer/screen: \"That payment didn't go through.\"", "Customer/screen: \"We couldn't start the payment. Please try again.\"", "Customer/screen: \"Your payment is still processing. We'll email you as soon as it clears.\"", "Customer/email: the decline email, carrying a way back in", "Customer/screen: after the window their attempt is gone and the flow restarts at step 1, with nothing saying why *(not built)*"], toldOnSuccess: ["Customer/screen: the confirmation screen", "Customer/email: ② the receipt, listing every file"], met: (s) => !!s.paidAt },
  ],
  new: [
    { what: "Receipt sent to the customer", next: "Send the receipt", from: "②", passive: true, records: [...sendRecords("② receipt → customer")], failures: [...sendFailures("② receipt → customer")], toldOnFail: ["Admin/screen: “The receipt to {customer} bounced — they may not know their submission arrived.” *(not built)*"], toldOnSuccess: ["Customer/email: ② the receipt"], met: sent("② receipt → customer") },
    { what: "Arrival announced", next: "Tell Admin it arrived", from: "②", passive: true, records: [...sendRecords("② arrival → Admin")], failures: [...sendFailures("② arrival → Admin")], toldOnFail: ["Admin/screen: a banner on the row — “Your arrival notice bounced. Check the address on your account.” *(not built)*"], toldOnSuccess: ["Admin/email: ② the arrival notice"], met: sent("② arrival → Admin") },
    { what: "Coach chosen", next: "Pick a coach", from: "assignedCoachId", act: "assign", toldOnFail: ["Admin/screen: “This has already gone to a coach. Reload to see where it is.” *(not built)*"], toldOnSuccess: ["Admin/screen: the row moves to Assigned and the coach's name appears on it"], met: (s) => !!s.assignedCoachId },
  ],
  assigned: [
    {
      ...
      toldOnFail: ["Admin/screen: \"no languages recorded for this coach\" on the row"], toldOnSuccess: ["Admin/screen: the translation prompt appears, or doesn't, per the language rule"], met: (_s, f) => f.coachHasLanguages,
    },
    {
      ...
      passive: true,
      toldOnFail: ["Admin/screen: “That did not go through — try again.” *(not built)*"], toldOnSuccess: ["Admin/screen: the row moves to Translating"], met: (_s, f) => f.files.intake_translation > 0,
    },
    { what: "Handed to the coach", next: "Hand to the coach", from: "③", act: "handoff", records: [...sendRecords("③ hand-off → coach")], failures: [...sendFailures("③ hand-off → coach")], toldOnFail: ["Admin/screen: “This has already gone to a coach. Reload to see where it is.” *(not built)*"], toldOnSuccess: ["Coach/email: ③ the hand-off, with a download link per file", "Admin/screen: the row moves to Sent"], met: sent("③ hand-off → coach") },
  ],
  intake_translating: [
    {
    ...
    { what: "Translated files uploaded", next: "Upload the translated files", from: "intake_translation", act: "uploadIntake", toldOnFail: ["Admin/screen: “That file was rejected — too large, wrong type, or empty.” *(not built)*"], toldOnSuccess: ["Admin/screen: the files appear in the folder"], met: has("intake_translation") },
  ],
  intake_translated: [
    { what: "Handed to the coach", next: "Hand to the coach", from: "③", act: "handoff", records: [...sendRecords("③ hand-off → coach")], failures: [...sendFailures("③ hand-off → coach")], toldOnFail: ["Admin/screen: “This has already gone to a coach. Reload to see where it is.” *(not built)*"], toldOnSuccess: ["Coach/email: ③ the hand-off, with a download link per file", "Admin/screen: the row moves to Sent"], met: sent("③ hand-off → coach") },
  ],
  sent_to_coach: [
    { what: "Hand-off emailed", next: "Email the hand-off", from: "③", passive: true, records: [...sendRecords("③ hand-off → coach")], failures: [...sendFailures("③ hand-off → coach")], toldOnFail: ["Admin/screen: a banner on the row — “They never received this. They do not know they have work waiting.” *(not built)*"], toldOnSuccess: ["Coach/email: ③ the hand-off"], met: sent("③ hand-off → coach") },
    {
      ...
      toldOnFail: ["Coach/screen: the download is gone — the folder was purged before they collected (410)"], toldOnSuccess: ["Coach/screen: the file downloads", "Admin/email: ④ picked up — the coach has it"], met: reached("in_review"),
    },
  ],
  in_review: [
    { what: "Response uploaded", next: "Upload the response", from: "response", act: "waitCoach", toldOnFail: ["Coach/screen: “That file was rejected — too large, wrong type, or empty.” *(not built)*"], toldOnSuccess: ["Coach/screen: the file appears in their folder", "Admin/email: ⑤ response submitted", "Coach/email: ⑤ the same notice, to the coach"], met: has("response") },
  ],
  awaiting_approval: [
    { what: "Admin and the coach told", next: "Tell Admin and the coach", from: "⑤", passive: true, records: [...sendRecords("⑤ response submitted → Admin + coach")], failures: [...sendFailures("⑤ response submitted → Admin + coach")], toldOnFail: ["Admin/screen: “That did not go through — try again.” *(not built)*"], toldOnSuccess: ["Admin/email: ⑤ response submitted", "Coach/email: ⑤ the same notice"], met: sent("⑤ response submitted → Admin + coach") },
    {
      ...
      act: "sendForTranslation",
      passive: true,
      toldOnFail: ["Admin/screen: “That did not go through — try again.” *(not built)*"], toldOnSuccess: ["Admin/screen: the row moves to Translating"], met: (_s, f) => f.files.response_translation > 0,
    },
    { what: "Approved and sent", next: "Approve and send", from: "feedbackEmailedAt", act: "approve", records: [...sendRecords("⑥ feedback ready → customer")], failures: [...sendFailures("⑥ feedback ready → customer"), "Refused — there is no response file to send"], toldOnFail: ["Admin/screen: “There is no response file to send yet.” *(not built)*"], toldOnSuccess: ["Customer/email: ⑥ feedback ready, stating the retention window", "Admin/screen: the row moves to Delivered"], met: (s) => !!s.feedbackEmailedAt },
  ],
  response_translating: [
    ...
    { what: "Translation uploaded", next: "Upload the translation", from: "response_translation", act: "uploadResponse", toldOnFail: ["Admin/screen: “That file was rejected — too large, wrong type, or empty.” *(not built)*"], toldOnSuccess: ["Admin/screen: the files appear in the folder"], met: has("response_translation") },
  ],
  response_translated: [
    { what: "Approved and sent", next: "Approve and send", from: "feedbackEmailedAt", act: "approve", records: [...sendRecords("⑥ feedback ready → customer")], failures: [...sendFailures("⑥ feedback ready → customer"), "Refused — there is no response file to send"], toldOnFail: ["Admin/screen: “There is no response file to send yet.” *(not built)*"], toldOnSuccess: ["Customer/email: ⑥ feedback ready, stating the retention window", "Admin/screen: the row moves to Delivered"], met: (s) => !!s.feedbackEmailedAt },
  ],
  complete: [
    { what: "Feedback emailed", next: "Email the feedback", from: "⑥", passive: true, records: [...sendRecords("⑥ feedback ready → customer")], failures: [...sendFailures("⑥ feedback ready → customer")], toldOnFail: ["Admin/screen: a banner on the row — “They never received this. They do not know they have work waiting.” *(not built)*"], toldOnSuccess: ["Customer/email: ⑥ feedback ready"], met: sent("⑥ feedback ready → customer") },
    {
      ...
      toldOnFail: ["Customer/screen: the download is gone — an operator purged the folder early (410)"], toldOnSuccess: ["Customer/screen: the file downloads", "Admin/email: ⑦ collected — the customer has it"], met: (s) => !!s.collectedAt,
    },
  ],
  collected: [
    { what: "Collection announced", next: "Tell Admin they collected", from: "⑦", passive: true, records: [...sendRecords("⑦ collected → Admin")], failures: [...sendFailures("⑦ collected → Admin")], toldOnFail: ["Admin/screen: “That did not go through — try again.” *(not built)*"], toldOnSuccess: ["Admin/email: ⑦ collected"], met: sent("⑦ collected → Admin") },
    { what: "Marked resolved", next: "Mark resolved", from: "trail · resolved", act: "resolve", toldOnFail: ["Admin/screen: “That did not go through — try again.” *(not built)*"], toldOnSuccess: ["Customer/email: ⑧ thank you, carrying the deletion date", "Admin/screen: the row moves to Resolved"], met: reached("resolved") },
  ],
  resolved: [
    { what: "Thank-you sent", next: "Send the thank-you", from: "⑧", passive: true, records: [...sendRecords("⑧ thank you → customer")], failures: [...sendFailures("⑧ thank you → customer")], toldOnFail: ["Admin/screen: “That did not go through — try again.” *(not built)*"], toldOnSuccess: ["Customer/email: ⑧ thank you"], met: sent("⑧ thank you → customer") },
    { what: "Deletion warning due", next: "Warning falls due", from: "deletionWarnedAt", act: "waitCron", failures: ["The sweep didn't run — CRON_SECRET unset, and it refuses rather than run unguarded"], toldOnFail: ["Admin/screen: “The nightly sweep has not run since {date}.” *(not built)*"], toldOnSuccess: ["Customer/email: ⑨ the deletion warning, a week out"], met: (s) => !!s.deletionWarnedAt },
  ],
  purge_imminent: [
    { what: "Warning sent", next: "Send the warning", from: "⑨", passive: true, records: [...sendRecords("⑨ deletion warning → customer")], failures: [...sendFailures("⑨ deletion warning → customer"), "Stamped even when the send failed — retrying nightly would turn one miss into seven"], toldOnFail: ["Admin/screen: “The deletion warning to {customer} did not send. They have no notice, and it will not retry.” *(not built)*"], toldOnSuccess: ["Customer/email: ⑨ the deletion warning"], met: sent("⑨ deletion warning → customer") },
    { what: "Files deleted", next: "Delete the files", from: "filesPurgedAt", act: "waitCron", failures: ["Storage refused the delete — the locator stays and the sweep retries *(not built)*"], toldOnFail: ["Admin/screen: “Storage refused the delete — {n} files are still there.” *(not built)*"], toldOnSuccess: ["Customer/screen: any link they kept now answers 410", "Admin/screen: the filenames show struck through in the folders"], met: (s) => !!s.filesPurgedAt },
  ],
  purged: [
    { what: "Bytes removed from storage", next: "Remove the bytes", from: "filesPurgedAt", toldOnSuccess: ["Admin/screen: the filenames show struck through in the folders"], met: (s) => !!s.filesPurgedAt },
    { what: "Locators cleared", next: "Clear the locators", from: "fileUrl = null", toldOnSuccess: ["Customer/screen: an old link answers 410 — gone, not missing"], met: (s) => !!s.filesPurgedAt },
    { what: "Record kept — permanently", next: "Keep the record", from: "the row survives", toldOnSuccess: ["Admin/screen: the row still says what was sent, forever"], met: () => true },
  ],
};
